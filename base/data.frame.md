...
df[do.call("order", unname(df[, -1])), ]      ; сортировка по всем столбцам кроме первого (unname разыменовывает вектор названий столбцов)

# сортировка в заданном порядке значений вектора `my_vec` — для этого преобразуем столбец в фактор с нужным порядком следования уровней
...
# перенос в начало строк с заданным значением "STR" — сортируем по логическому вектору (логический вектор сортируется от FALSE до TRUE)
df[order(df$COL != "STR"),]
