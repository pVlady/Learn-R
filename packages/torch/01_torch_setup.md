...
Кроме самого пакета `torch` устанавливаем следующие пакеты:
* `torchvision` для работы с изображениями
* `torchaudio` для работы со звуком
* `luz` — пакет который предоставляет удобную обертку для выполнения операций по обучению нейросетей
```r
