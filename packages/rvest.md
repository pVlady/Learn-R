...
node <- html_nodes(page, css = "#update_date")             ; поиск элемента с указанным атрибутом-идентификатором id
node <- html_nodes(page, xpath = '//*[@id="update_date"])  ; тоже, ислопользуя xpath-локатор вместо css-селектора 
node <- html_nodes(page, css = ".simple_text")             ; поиск элемента по классу (атрибут class)
node <- html_nodes(page, css = "[title]")                  ; поиск элемента по наличию атрибута title
node <- html_nodes(page, css = "[title]='part1'")          ; поиск элемента по значению атрибута title
node <- html_nodes(page, css = "table")                    ; поиск элемента по тегу table
node <- html_nodes(page, css = "div p a")                  ; поиск по вложенным тегам (ссылка в параграфе в блоке div)
```
...
node <- html_nodes(page, css = "table") %>%               ; получить таблицу
