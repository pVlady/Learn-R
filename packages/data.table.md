...
При операторном соединении рекомендуется также указать аргумент `j` для возвращения только нужных столбцов, поскольку в противном случае возвращаются *все* столбцы обеих таблиц.\
