...
- При соединении таблиц вида `a[b]` можно не указывать аргумент `on`, если таблица `a` имеет ключ — соединение будет выполняться по совпадения ключа таблицы `a` с **начальными столбцами** `b`.
- Выражение вида `a[b, bb:=i.B]` добавит в таблицу `a` столбец `bb` со значениями столбца `B` из таблицы `b`. Там, где не будет совпадения по ключу, добавятся значения `NA`. При этом новых строк в `a`, которые есть в `b` (но нет в `a`), добавлено не будет.\
То же самое будет в случае, если `b` является списком именованных векторов, а не объектом data.table.
