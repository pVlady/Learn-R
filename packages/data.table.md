...
- Выражение вида `a[b, bb:=i.B]` добавит в таблицу `a` столбец `bb` со значениями столбца `B` из таблицы `b`. Где не будет совпадения по ключу, добавятся значения `NA`. То же самое будет в случае, если `b` является списком, а не объектом data.table. При этом и в том и в другом случае новых строк в `a`, которые есть в `b` (но нет в `a`), добавлено не будет.
