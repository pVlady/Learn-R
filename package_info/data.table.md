...
* Чтобы вернуть столбцы data.table, которые заданы значениями переменной x необходимо использовать `dt[, x, with = FALSE] (иначе будет искать стоолбец с названием `x`)
