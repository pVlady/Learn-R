...
* Для каждой таблицы загруженной с помощью `readRDS()` или `load()` рекомендуется вызввать функцию `setalloccol()`, поскольку сохранение таблиц в бинарном формате нарушает оптимизацию их размещения в RAM.
